...
import { MapPin, Search, ChevronDown, Percent, Clock, Star, History } from 'lucide-react';
...
export const Home: React.FC<HomeProps> = ({ onSelectRestaurant, onViewHistory, onViewProfile, onOpenSearch }) => {
    ...
    >
            ...
            <h1 className="text-2xl font-black tracking-tight lowercase text-slate-800 dark:text-slate-100 flex items-baseline" style={{ fontFamily: 'Outfit, sans-serif' }}>
              {"cra".split("").map((char, index) => (
                 <motion.span
                   key={index}
                   animate={{ y: [0, -3, 0] }}
                   transition={{ duration: 2, repeat: Infinity, delay: index * 0.1, ease: "easeInOut" }}
      ...
      <div className="px-5 pb-8 pt-4">
        ...
        <div className="flex overflow-x-auto no-scrollbar gap-4 pb-6 -mx-5 px-5">
          ...
          >
            <div className="relative z-10">
              <span className="font-bold text-2xl text-transparent bg-clip-text bg-gradient-to-r from-yellow-300 to-yellow-500">PRO</span>
              <p className="text-white/90 text-sm mt-1 mb-3">Free delivery + 20% extra off</p>
              <span className="bg-white/20 backdrop-blur-md px-3 py-1.5 rounded-full text-xs font-semibold">Join Now</span>
            </div>
          </motion.div>
        </div>
              ...
              >
                ...
                <div className="p-4 bg-white dark:bg-slate-900">
                  ...
                  <div className="flex items-center gap-4 mt-2 text-slate-500 dark:text-slate-400 text-sm font-medium">
                    ...
                    <div>{restaurant.tags[0]}, {restaurant.tags[1]}</div>
                  </div>
                </div>
              </motion.div>
            ))}
