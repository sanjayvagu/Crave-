...
import { ArrowLeft, MapPin, Receipt, Clock, ChevronRight, Plus, Minus, CheckCircle2, TicketPercent, XCircle, Tag, X } from 'lucide-react';
...
const NumberTicker = ({ value }: { value: number }) => {
  return (
    <span className="relative inline-block overflow-hidden" style={{ width: 'ch', minWidth: '4ch' }}>
...
export const Cart: React.FC<CartProps> = ({ cart, onBack, onCheckoutComplete, onUpdateCart }) => {
  ...
  return (
    ...
    >
            ...
            {!appliedCoupon ? (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-green-800 font-bold text-sm tracking-tight">'{appliedCoupon}' applied</p>
                    <p className="text-green-600 text-xs font-medium mt-0.5">You saved ${discountAmount.toFixed(2)}</p>
                  </div>
                </div>
                <button 
          ...
          <div className="space-y-2 text-sm">
            ...
            <AnimatePresence>
              {discountAmount > 0 && (
                ...
                >
                  <span className="flex items-center gap-1">Item Discount <Tag className="w-3 h-3"/></span>
                  <span className="flex items-center">-$<NumberTicker value={discountAmount} /></span>
                </motion.div>
              )}
            </AnimatePresence>
            <div className="flex justify-between text-slate-600 dark:text-slate-300">
            ...
            <AnimatePresence>
              {tipPercentage > 0 && (
                ...
                >
                  ...
                  <span className="flex items-center">$<NumberTicker value={tipAmount} /></span>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
      ...
      <div className="bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 px-5 pt-6 pb-32 shrink-0 rounded-t-3xl shadow-[0_-10px_40px_rgb(0,0,0,0.05)]">
        ...
        >
          <span className="flex items-center">Pay $<NumberTicker value={total} /></span>
