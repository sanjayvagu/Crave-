...
export const SearchScreen: React.FC<SearchScreenProps> = ({ onSelectRestaurant }) => {
  ...
  return (
    ...
    >
      <div className="pt-12 px-5 pb-4 bg-white dark:bg-slate-900 shadow-sm z-10 shrink-0">
         ...
         <AnimatePresence>
           {!query ? (
             <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
               <h3 className="font-bold text-slate-800 dark:text-slate-100 mb-4 text-lg tracking-tight">Trending Searches</h3>
             ...
             <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
               <h3 className="font-bold text-slate-800 dark:text-slate-100 mb-4 text-lg tracking-tight">Results</h3>
               {results.length > 0 ? (
                   ...
                   {results.map((r, idx) => (
                     ...
                     >
                       ...
                       <div>
                         ...
                         <p className="text-sm text-slate-500 dark:text-slate-400 line-clamp-1">{r.tags.join(', ')}</p>
                       </div>
                     </motion.div>
                   ))}
                 </div>
