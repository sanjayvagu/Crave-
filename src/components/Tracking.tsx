...
import { Home, CheckCircle2, ChefHat, Bike, MapPin, Navigation, Phone, X } from 'lucide-react';
...
const STATUSES = [
  { id: 'accepted', label: 'Order Placed', icon: CheckCircle2, delay: 0, subtext: 'Restaurant has received your order' },
  { id: 'preparing', label: 'Preparing', icon: ChefHat, delay: 2000, subtext: 'Your food is being prepared' },
  { id: 'ontheway', label: 'Out for Delivery', icon: Bike, delay: 5000, subtext: 'Delivery partner has picked up your order' },
  { id: 'delivered', label: 'Delivered', icon: MapPin, delay: 9000, subtext: 'Enjoy your food!' },
];

export const Tracking: React.FC<TrackingProps> = ({ onGoHome }) => {
  const [currentStep, setCurrentStep] = useState(0);
  const [timeLeft, setTimeLeft] = useState(() => Math.floor(Math.random() * (30 * 60 - 20 * 60 + 1) + 20 * 60));
                ...
                <div key={status.id} className="flex flex-col items-center relative">
                  <motion.div 
                    ...
                    animate={{ 
                      ...
                      borderColor: isCurrent ? '#60b246' : isCompleted ? '#60b246' : '#e2e8f0',
           ...
           <AnimatePresence mode="wait">
             ...
             >
                <div className="w-14 h-14 rounded-full bg-white shadow-[0_4px_20px_rgb(0,0,0,0.05)] border border-slate-100 flex items-center justify-center shrink-0">
                   ...
                   >
                     {React.createElement(STATUSES[currentStep].icon, { className: "w-7 h-7" })}
                   </motion.div>
                </div>
                <div>
